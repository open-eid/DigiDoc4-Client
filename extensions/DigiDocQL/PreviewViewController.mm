...
static std::string htmlEntityEncode(std::string s)
{
...
dl { margin-bottom: 10px };
        ...
        [h appendFormat:@"<h2>%s<hr size='1' /></h2>", htmlEntityEncode(request.fileURL.lastPathComponent.UTF8String).c_str()];
        try
        {
            ...
            for (const DataFile *doc : d->dataFiles()) {
                [h appendFormat:@"<li>%s</li>", htmlEntityEncode(doc->fileName()).c_str()];
            }
            [h appendString:@"</ol>"];

            [h appendString:@"<font>Signatures</font>"];
            for (const Signature *s : d->signatures()) {
                [h appendFormat:@"<dl><dt>Signer</dt><dd>%s</dd>", htmlEntityEncode(s->signedBy()).c_str()];
                ...
                if (!roles.empty()) {
                    [h appendFormat:@"<dt>Role</dt><dd>%s&nbsp;</dd>", htmlEntityEncode(roles).c_str()];
                }
                if (!s->countryName().empty()) {
                    [h appendFormat:@"<dt>Country</dt><dd>%s&nbsp;</dd>", htmlEntityEncode(s->countryName()).c_str()];
                }
                if (!s->city().empty()) {
                    [h appendFormat:@"<dt>City</dt><dd>%s&nbsp;</dd>", htmlEntityEncode(s->city()).c_str()];
                }
                if (!s->stateOrProvince().empty()) {
                    [h appendFormat:@"<dt>State</dt><dd>%s&nbsp;</dd>", htmlEntityEncode(s->stateOrProvince()).c_str()];
                }
                if (!s->postalCode().empty()) {
                    [h appendFormat:@"<dt>Postal code</dt><dd>%s&nbsp;</dd>", htmlEntityEncode(s->postalCode()).c_str()];
                }
                [h appendString:@"</dl>"];
            }
