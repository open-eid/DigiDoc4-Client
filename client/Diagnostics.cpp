...
void Diagnostics::generalInfo(QTextStream &s) const
{
	...
	s << "<b>" << tr("Arguments:") << "</b> " << qApp->arguments().join(' ') << "<br />"
		...
		<< "<br />RPUUID: " << (QSettings().value(QStringLiteral("MIDUUID-CUSTOM"), false).toBool() && QSettings().contains(QStringLiteral("MIDUUID")) ? tr("is set manually") : tr("is set by default"))
#else
#ifdef MOBILEID_URL
